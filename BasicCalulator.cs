...
namespace ConsoleProject
{
    class BasicCalulator
    {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < charInputArray.Length; i++)
                {
                    ...
                    else if (charInputArray[i] == ')')
                    {
                        ...
                        strInput = strInput.Replace(strInput.Remove(iCloseBracketPos, strInput.Length - 1).Remove(0, iOpenBracketPos), Calculate(strInput.Remove(iCloseBracketPos , strInput.Length - 1).Remove(0, iOpenBracketPos)));
                    }
        ...
        String Calculate(String strToCalculate)
        {
            ...
            {
                strToCalculate.Replace("+", "_").Replace("-", "_").Replace("/", "_").Replace("*", "_");
                strToCalculateModSplit = strToCalculateMod.Split('_');
                 

                for (int i = 0; i < strToCalculateModSplit.Length - 1; i++)
                {
                    strToCalculateOperator = strToCalculateModSplit[i].Split('^');
                    decResult = System.Convert.ToDecimal(strToCalculateModSplit[0]);
                    for (int e = 0; e < strToCalculateOperator.Length - 1; e++)
                    {
                        decResult = (Decimal)Math.Pow((double)decResult, (double)System.Convert.ToDecimal(strToCalculateModSplit[i + 1]));
                    }
                }
            }
            else if (true)
